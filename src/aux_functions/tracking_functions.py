...
class KalmanBoxTracker(object):
  ...
  def __init__(self,bbox):
    ...
    # Test to avoid orientation correction
    # self.kf.F = np.array([[1,0,0,0,0,1,0,0,0],  # x
    #                       [0,1,0,0,0,0,1,0,0],  # y
    #                       [0,0,1,0,0,0,0,1,0],  # s
    #                       [0,0,0,1,0,0,0,0,0],  # r
  ...
  def update(self,bbox):
    ...
    if self.kf.x[4] >= np.pi: self.kf.x[4] -= np.pi * 2    # make the theta still in the range
    if self.kf.x[4] < -np.pi: self.kf.x[4] += np.pi * 2
    ...
    if abs(new_theta - self.kf.x[4]) >= np.pi * 3 / 2.0:
      if new_theta > 0: self.kf.x[4] += np.pi * 2
      else: self.kf.x[4] -= np.pi * 2

    #########################     # flip

    self.kf.update(sort_functions.convert_bbox_to_z(bbox))

    # if self.kf.x[4] >= np.pi: 
    #   self.kf.x[4] -= np.pi * 2 # make the theta still in the rage
    # if self.kf.x[4] < -np.pi: 
    #   self.kf.x[4] += np.pi * 2
